...
class IncorrectIDStyleError(maxdbException) :
	def __init__(self,value):
		maxdbException.__init__(self,'This primary key name  is not in the expected style,please rename the column to %s or switch to another style'%(value))

class StyleMismatchError(maxdbException):
	def __init__(self, value):
		maxdbException.__init__(self,'The name %s is only permitted for primary key,change the column name or switch to another style'%value)


class PrimaryKeyNotFounded(maxdbException):
	def __init__(self, value):
		maxdbException.__init__(self,"No primary key was defined on table %s"%(value))
...
class MaxdbConnection(DBAPI):
	...
	def __init__ (self, user, password, database, host = '',autoCommit=1,sqlmode='internal',isolation=None,timeout=None, **kw):
	    global dbapi
	    if dbapi is None:
	...
	def _setAutoCommit(self, conn, auto):
		conn.close()
		conn.__init__(self.user, self.password, self.database, self.host,**self._getConfigParams(self.sqlmode,auto))
		
	 
	
	 
	def createSequenceName(self,table):
		""" sequence name are builded with the concatenation of the table name with '_SEQ' word
	...
	def createTable(self, soClass):
		...
		self.query('CREATE TABLE %s (\n%s\n)' % \
		 		           (soClass._table, self.createColumns(soClass)))
		self.query("CREATE SEQUENCE %s" % self.createSequenceName(soClass._table))
	...
	GET_COLUMNS   = (" SELECT COLUMN_NAME,NULLABLE,DATA_DEFAULT,DATA_TYPE, DATA_LENGTH,DATA_SCALE FROM USER_TAB_COLUMNS WHERE TABLE_NAME=UPPER('%s')")
	GET_PK_AND_FK = (""" SELECT constraint_cols .column_name,constraints.constraint_type , refname,reftablename FROM
					  user_cons_columns constraint_cols 
					  INNER JOIN user_constraints constraints  ON  constraint_cols.constraint_name=constraints.constraint_name 
					  LEFT OUTER JOIN show_foreign_key fk on constraint_cols.column_name=fk.columnname
	...
	def columnsFromSchema(self, tableName, soClass):
		...
		if len(pkmap) == 0: raise PrimaryKeyNotFounded, tableName
		
		for field, nullAllowed, default, data_type, data_len, data_scale in colData:
	...
	def guessClass(self, t, flength, fscale=None):
		...
		#To get the data call the read member with desired size (default =-1 means get all)
