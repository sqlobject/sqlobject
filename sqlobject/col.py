...
#         cls.sqlTypeRegistry = cls.sqlTypeRegistry.copy()

#     def registerSQLType(cls, dbname, type_func):
#         if not isinstance(dbname, (list, tuple)):
#             dbname = [dbname]
#         for name in dbname:
#             cls.sqlTypeRegistry[name] = type_func
#     registerSQLType = classmethod(registerSQLType)

#     def __init__(self, added_class, attr_name,
#                  dbName=None,
#                  default=NoDefault,
#                  alternateID=False,
#                  alternateMethodName=None,
...
#                 "Name must be SQL-safe (letters, numbers, underscores): "
#                 "%s (or use forceDBName=True)" % repr(name))
#         assert attr_name != 'id', (
#             "The column name 'id' is reserved for SQLObject use (and is "
#             "implicitly created).")
#         assert name, "You must provide a name for all columns"
#         self.immutable = immutable
#         assert cascade in (None, True, False), (
#             "The cascade attribute must be one of None, True, or False "
#             "(you gave: %r" % cascade)
#         self.cascade = cascade
...
#             assert (notNone is NoDefault or
#                     (not notNone) == (not notNull)), (
#                 "The notNull and notNone arguments are aliases, and must "
#                 "not conflict.  You gave notNull=%r, notNone=%r"
#                 % (notNull, notNone))
#         elif notNone is not NoDefault:
#             self.notNone = notNone
#         if self.notNone:
...
#         # and assume that.
#         if dbName is None:
#             self.dbName = added_class.sqlmeta.style.pythonAttrToDBColumn(self.name)
#         else:
#             self.dbName = dbName
...
#         self.soClass.addColumn(self.name, self)
...
#     def createSQL(self, dbname):
#         if self.customSQLType is not None:
#             return self.customSQLType
#         try:
#             func = self.sqlTypeRegistry[dbname]
#         except KeyError:
#             assert 0, (
#                 "The database %s does not support the column type %s "
#                 "(no SQL type generator found; only %s found)"
#                 % (dbname, self.__class__.__name__,
#                    ', '.join(self.sqlTypeRegistry.keys())))
#         else:
#             return func(self, dbname)
...
#                 "This object has been deleted from the database: %r"
#                 % obj)
#         if obj.sqlmeta.cacheColumns:
#             columns = obj.sqlmeta._columnCache
#             if columns is None:
...
#     factory_class = BoundCol

#     def addToClass(self, cls, addToThisClass, name):
#         me = self or cls
#         me.__addtoclass__(addToThisClass, name)
#     addToClass = declarative.classinstancemethod(addToClass)

# class BoundStringCol(BoundCol):
...
#         if not self.length:
#             assert self.varchar == 'auto' or not self.varchar, (
#                 "Without a length strings are treated as TEXT, not "
#                 "VARCHAR")
#             self.varchar = False
#         elif self.varchar == 'auto':
#             self.varchar = True
#         BoundCol.__init__(self, *args, **kw)
