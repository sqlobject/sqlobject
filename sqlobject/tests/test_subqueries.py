...
def test_1syntax_in():
    ...
    assert str(select) == \
        "SELECT test_in1.id, test_in1.col1 " \
...
def test_3syntax_exists():
    setup()
    select = TestIn1.select(NOTEXISTS(
        Select(TestIn2.q.col2,
               where=(Outer(TestIn1).q.col1 == TestIn2.q.col2))))
    assert str(select) == \
        "SELECT test_in1.id, test_in1.col1 " \
        "FROM test_in1 WHERE NOT EXISTS " \
        "(SELECT test_in2.col2 FROM test_in2 " \
        "WHERE ((test_in1.col1) = (test_in2.col2)))"

    setupClass(TestOuter)
    select = TestOuter.select(NOTEXISTS(
        Select(TestIn1.q.col1,
               where=(Outer(TestOuter).q.fk == TestIn1.q.id))))
    assert str(select) == \
        "SELECT test_outer.id, test_outer.fk_id " \
        "FROM test_outer WHERE NOT EXISTS " \
...
def test_4syntax_direct():
    ...
    assert str(select) == \
        "SELECT test_in1.id, test_in1.col1 " \
        "FROM test_in1 WHERE ((test_in1.col1) = " \
        "(SELECT test_in2.col2 FROM test_in2 " \
...
def test_6syntax_join():
    ...
    select = TestIn1.select(TestIn1.q.col1 == Select(TestIn2.q.col2,
                            where=(TestIn2.q.col2 == "test"), join=j))
    assert str(select) == \
        "SELECT test_in1.id, test_in1.col1 " \
        "FROM test_in1 WHERE ((test_in1.col1) = " \
        "(SELECT test_in2.col2 FROM test_in2 " \
